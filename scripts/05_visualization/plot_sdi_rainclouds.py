...
def main():
    ...
    fig.subplots_adjust(wspace=0.4, left=0.06, right=0.98, top=0.85, bottom=0.25)
    ...
    for ax, (ds, cond_a, cond_b, paired, color, label) in zip(axes, PANELS):
        ...
        for i, (p_raw, p_fdr, m) in enumerate(zip(ps_raw, ps_fdr, means)):
            ...
            ax.text(i, ypos, marker, ha="center", va="bottom" if m>=0 else "top",
                    fontsize=15 if bold else 14, fontweight="bold" if bold else "normal", color="black")
            
        ax.set_title(f"{label}", fontsize=19, fontweight="bold", pad=20)
        ax.set_xlabel("")
        if ax is axes[0]:
        ...
        x = np.arange(len(NET_SHORT))
        ax.set_xticks(x)
        ax.set_xticklabels(NET_SHORT, rotation=45, ha="right", fontsize=15)
    ...
    fig.legend(handles=legend_elements, loc="lower center", ncol=4, fontsize=14, framealpha=0.0, bbox_to_anchor=(0.5, -0.02))
