...
for label, csv_key, cond_a, cond_b, is_paired, letter in DATASETS:
    ...
    # ── dFC: compute from per-subject CSVs ───────────────────────────────
        ...
        for net in NETWORKS:
            ...
            if is_paired:
                ...
                val = (a - b).loc[common].mean() if len(common) > 0 else 0.0
                delta_se.append(val)
                if len(common) > 1:
                    stat, p = stats.wilcoxon((a - b).loc[common])
                else:
                    p = 1.0
                ps_raw.append(p)
            else:
                ...
                val = np.mean(a) - np.mean(b) if len(a) > 0 and len(b) > 0 else 0.0
                delta_se.append(val)
    ...
    for roi_id, net_idx in roi_to_net.items():
        ...
        if metric != "dSW" and ps_raw[net_idx] >= 0.05:
            val = val * 0.15  # Fade out non-significant networks for dFC
    ...
    if metric == "dSW":
        ...
        # Symmetric individual scaling for dFC (with floor of 0.02)
...
for i, (label, csv_key, cond_a, cond_b, is_paired, letter) in enumerate(DATASETS):
    ...
    cb.set_label(f'ΔSE {metric}', fontsize=15, fontweight='bold', labelpad=5)
