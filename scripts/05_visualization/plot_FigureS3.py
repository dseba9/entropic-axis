...
def main():
    ...
    for i, (label, csv_key, cond_a, cond_b, is_paired, color) in enumerate(COLS):
        # 1. Load data
        csv_path = os.path.join(csv_dir, f"{csv_key}_dFC_SE_LONO_SubjectCondition.csv")
        ...
        cb.set_label(r'$\Delta$SE(dFC)', fontsize=15, fontweight='bold', labelpad=4)
        ...
        # Add significance markers with enhanced visibility (larger font, better placement)
        ...
        if i == 0:
            axes_rc[0].set_ylabel(r"$\Delta$SE(dFC)", fontsize=17, fontweight="bold")
        else:
    ...
    out_png = os.path.join(out_dir, "FigureS3.png")
    out_svg = os.path.join(out_dir, "FigureS3.svg")
    
    plt.savefig(out_png, dpi=300, bbox_inches='tight')
    plt.savefig(out_svg, bbox_inches='tight')
    plt.close()
    
    print(f"Composite S3 figure saved successfully to:")
    print(f"  {out_png}")
    print(f"  {out_svg}")
